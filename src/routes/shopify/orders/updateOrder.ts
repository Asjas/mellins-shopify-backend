...
export default async function updateOrderHandler(request: FastifyRequest, reply: FastifyReply) {
  const {
    id: orderId,
  ...
  if (isIBT && !orderMetafieldsData.tags.includes(SHOPIFY_TAGS.IBT_REQUESTED)) {
    ...
    this.nodemailer.sendMail(message, (err, info) => {
      ...
      this.log.info(info);
      this.log.info(`Contact Lens order #${orderNumber} refund request email emailed to Luzanne.`);
    });
  }
  ...
  if (isVoucher && !orderMetafieldsData.tags.includes(SHOPIFY_TAGS.VOUCHER_REQUESTED)) {
    ...
    this.nodemailer.sendMail(message, (err, info) => {
      ...
      this.log.info(info);
      this.log.info(`Contact Lens order #${orderNumber} voucher request email emailed to Colette.`);
    });
  }
  ...
  if (
    ...
    this.nodemailer.sendMail(message, (err, info) => {
      ...
      this.log.info(info);
      this.log.info(`Contact Lens order #${orderNumber} collection email emailed to Lauren Rheeder.`);
    });
